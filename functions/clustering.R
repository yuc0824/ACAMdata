...
self_clustering <- function(inputTags, percent_dropout = 10, tsne_min_cells = 200, dimensions = 3,
  ...
  cluster_results <- rbind(cluster_results, matrix(c(sc3OUTPUT), 
                                                   nrow = 1, byrow = TRUE))
  ...
  cluster_results <- rbind(cluster_results, matrix(c(cidrOUTPUT@clusters), 
                                                   nrow = 1, byrow = TRUE))
  ...
  cluster_results <- rbind(cluster_results, matrix(c(seurat_output), 
                                                   nrow = 1, byrow = TRUE))
  ...
  cluster_results <- rbind(cluster_results, matrix(c(tsne_kmeansOUTPUT$cluster), 
                                                   nrow = 1, byrow = TRUE))
  ...
  rownames(cluster_results) <- c("SC3", "CIDR", 
  ...
  for (i in c("SC3", "CIDR", "Seurat", 
    ...
    for (j in c("SC3", "CIDR", "Seurat", 
                "tSNE+kmeans", "SIMLR")) {
      ARI[i, j] <- adjustedRandIndex(unlist(cluster_results[i, 
      ]), unlist(cluster_results[j, ]))
    }
  }
